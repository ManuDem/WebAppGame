...
describe('MockMatchState i18n', () => {
    ...
    test('bundle mock longNames mantiene prefisso localizzato', () => {
        const bundle = createMockMatchBundle('my_turn', 'en', { longNames: true });
        const me = bundle.state.players.get(bundle.localSessionId);
        expect(me?.username.startsWith(t('en', 'qa_mock_local_name'))).toBe(true);
        expect((me?.username ?? '').length).toBeGreaterThan(t('en', 'qa_mock_local_name').length);
    });
