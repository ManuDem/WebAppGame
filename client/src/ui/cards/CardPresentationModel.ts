...
export function buildMiniCardInfo(card: ICardData, tr: CardTranslateFn): string {
    const shortDesc = getCompactSource(card);
    const type = String(card.type ?? '').toLowerCase();

    if (type === CardType.HERO || type === CardType.EMPLOYEE) {
        if (typeof card.targetRoll === 'number') return tr('card_mini_hero_roll', { value: card.targetRoll });
        return shortDesc ? tr('card_mini_hero_source', { value: shortDesc }) : tr('card_mini_hero_default');
    }
    ...
    if (type === CardType.ITEM || type === CardType.OGGETTO) {
        return shortDesc ? tr('card_mini_item_source', { value: shortDesc }) : tr('card_mini_item_default');
    }
    if (type === CardType.CHALLENGE || type === CardType.MODIFIER) {
        return shortDesc ? tr('card_mini_reaction_source', { value: shortDesc }) : tr('card_mini_reaction_default');
    }
    if (type === CardType.MAGIC || type === CardType.EVENTO) {
        return shortDesc ? tr('card_mini_magic_source', { value: shortDesc }) : tr('card_mini_magic_default');
    }
    return shortDesc || tr('card_mini_special');
}
...
export function buildInspectPresentation(card: ICardData, tr: CardTranslateFn): InspectPresentation {
    const typeLabel = localizeCardType(card, tr);
    const title = String(card.name ?? tr('game_card_unknown'));
    ...
    const fullDesc = String(card.description ?? '').trim();
...
function getCompactSource(card: ICardData): string {
    const shortDesc = typeof (card as any)?.shortDesc === 'string'
        ? String((card as any).shortDesc).trim()
        : '';
    const full = String(card.description ?? '').trim();
