...
const requestedUrl = process.env.QA_URL || "http://localhost:3000/";
...
function getSafeArea(width, height) {
  ...
  if (tier === "A" || tier === "B") return { top: 12, right: 12, bottom: 12, left: 12, tier };
  if (tier === "C") return { top: 8, right: 10, bottom: 8, left: 10, tier };
...
async function collectLayoutIssues(page, viewport) {
  ...
  return page.evaluate(({ safeArea }) => {
    ...
    if (!app) issues.push("#app missing");
    else checkRect("#app", app.getBoundingClientRect(), { enforceSafeArea: false });

    const canvas = document.querySelector("canvas");
    if (!canvas) issues.push("canvas missing");
    else checkRect("canvas", canvas.getBoundingClientRect(), { enforceSafeArea: false });

    const uiRoot = document.querySelector('#ui-root');
    if (uiRoot) checkRect('#ui-root', uiRoot.getBoundingClientRect(), { enforceSafeArea: false });

    const domInputs = Array.from(document.querySelectorAll("input[type='text']")).filter(isVisibleElement);
    domInputs.forEach((input, index) => checkRect(`input[${index}]`, input.getBoundingClientRect()));
    ...
    return { issues, safeArea, viewport: { width: vw, height: vh } };
  }, { safeArea });
}
...
async function resolveBaseUrl(browser) {
  const candidates = buildCandidateUrls(requestedUrl);
...
const hardTimeout = setTimeout(() => {
  console.error(`FATAL: responsive capture hard-timeout after ${hardTimeoutMs}ms`);
  process.exit(4);
}, hardTimeoutMs);
...
const url = await resolveBaseUrl(browser);
await mkdir(outputDir, { recursive: true });

const targets = [];
for (const lang of languages) {
  targets.push({ key: `boot-${lang}`, url: withQuery(url, { lang, qaScreen: "boot" }), settleMs: 350 });
  targets.push({ key: `login-${lang}`, url: withQuery(url, { lang, qaScreen: "login" }), settleMs: waitMs });
  targets.push({ key: `prelobby-${lang}`, url: withQuery(url, { lang, qaScreen: "prelobby", qaPreLobby: 1 }), settleMs: waitMs });
}
...
await browser.close();
await writeFile(`${outputDir}/summary.initial.json`, JSON.stringify(summary, null, 2), "utf8");
clearTimeout(hardTimeout);

const failing = summary.filter((entry) => !entry.ok);
console.log("===RESPONSIVE_CAPTURE_CHANNEL===");
console.log(channel);
console.log("===RESPONSIVE_CAPTURE_URL===");
console.log(url);
console.log("===RESPONSIVE_CAPTURE_SUMMARY===");
console.log(JSON.stringify(summary, null, 2));
if (failing.length > 0) {
  console.error(`RESPONSIVE_CAPTURE_ASSERT_FAILED (${failing.length} entries)`);
  process.exitCode = 2;
}
