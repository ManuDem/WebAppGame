...
class OfficeRoom extends colyseus_1.Room {
    ...
    onJoin(client, options, auth) {
        ...
        player.username = auth?.ceoName || options?.username || `CEO_${client.sessionId.substring(0, 4)}`;
    ...
    async onLeave(client, consented) {
        ...
        if (player) {
            ...
            if (this.state.phase !== SharedTypes_1.GamePhase.WAITING_FOR_PLAYERS && !consented) {
                try {
    ...
    // ─────────────────────────────────────────────────────
    startReactionWindow(triggerPlayerId, actionType, targetCardId, targetCrisisId) {
        ...
        if (targetCrisisId)
            pending.targetCrisisId = targetCrisisId;
        pending.timestamp = Date.now();
        this.state.pendingAction = pending;
        this.state.reactionEndTime = pending.timestamp + SharedTypes_1.REACTION_WINDOW_MS;
        ...
        }, SharedTypes_1.REACTION_WINDOW_MS);
    }
    resolveReactions() {
    ...
    handleJoinGame(client, _data) {
        ...
        if (this.state.playerOrder.length >= MIN_PLAYERS) {
            let allReady = true;
            this.state.players.forEach(p => { if (!p.isReady)
                allReady = false; });
            if (allReady) {
                this.startGame();
            }
    ...
    handleDrawCard(client) {
        ...
        const player = this.state.players.get(client.sessionId);
        if (player) {
            const cardSchema = new State_1.CardState();
            ...
            if (drawnCard.costPA !== undefined)
                cardSchema.costPA = drawnCard.costPA;
            if (drawnCard.isFaceUp !== undefined)
                cardSchema.isFaceUp = drawnCard.isFaceUp;
    ...
    handlePlayEmployee(client, data) {
        ...
        if (!this.checkPlayerTurnAction(client, 1))
            return;
        console.log(`🃏 [STUB] PLAY_EMPLOYEE by ${client.sessionId}`);
        this.startReactionWindow(client.sessionId, SharedTypes_1.ClientMessages.PLAY_EMPLOYEE, data?.cardId);
    }
    handleSolveCrisis(client, data) {
        // Assume solving a crisis costs 2 PA
        if (!this.checkPlayerTurnAction(client, 2))
            return;
        console.log(`💼 [STUB] SOLVE_CRISIS by ${client.sessionId}`);
        this.startReactionWindow(client.sessionId, SharedTypes_1.ClientMessages.SOLVE_CRISIS, undefined, data?.crisisId);
    }
    handlePlayMagic(client, data) {
        ...
        client.send(SharedTypes_1.ServerEvents.ERROR, { message: "Not implemented (FASE 3)" });
    }
    handlePlayReaction(client, data) {
        if (this.state.phase !== SharedTypes_1.GamePhase.REACTION_WINDOW) {
            client.send(SharedTypes_1.ServerEvents.ERROR, { message: "Nessuna finestra di reazione attiva." });
            return;
        }
        if (client.sessionId === this.state.pendingAction?.playerId) {
            client.send(SharedTypes_1.ServerEvents.ERROR, { message: "Non puoi reagire alla tua stessa azione." });
            return;
        }
